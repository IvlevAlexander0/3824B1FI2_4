...
class LoggingOperations // Класс для логирования количества произведенных операций пользователем.
{
private:
	struct InputLogs // Структура данных, подаваемых логгеру на вход при записи логов.
	{
...
public:
	...
	void add(const std::string& tableName_, const std::string& operationName_, size_t operationsCount) // Добавление лога в набор логов.
	{
	...
	void writeToFile(const std::string& fileName) const // Сохранение логов в файл.
	{
		std::ofstream fout; // Создание потока для записи в файл.
		fout.open(fileName); // Открытие файла для записи с именем fileName. Если файл не существует - он создастся автоматически.
		...
		if (fout.is_open()) // is_open() - проверяет, открылся ли файл.
		{
			...
			{
				fout << logs[i].tableName << " " << logs[i].operationName << " " << logs[i].operationsCount << std::endl; // Запись строк логов в файл.
			}

			// Вывод общего числа операций каждого действия:
			fout << std::endl << "+------------------+TOTAL ACTIONS+------------------+" << std::endl;
			...
			fout << std::endl << "+---------------+TOTAL FOR EACH TABLE+---------------+" << std::endl;
		...
		{
			std::cerr << "The file did not open!" << std::endl; // Вывод сообщения об ошибке открытия файла.
		}
	}
	...
	void print() const // Печать текущих логов в консоль.
	{
		...
		for (size_t i = 0; i < logs_size; ++i)
		{
			std::cout << logs[i].tableName << " " << logs[i].operationName << " " << logs[i].operationsCount << "\n"; // Вывод строк логов в консоль.
		}
	}
...
public:
	...
	size_t insertALL(const TKey& key, const TValue& value) // Вставка записи сразу во все 3 типа таблиц.
	{
		...
		return opCountUnordered + opCountAVL + opCountHash; // Возвращаем количество затраченных операций в сумме.
	}

	FindResult findALL(const TKey& key) // Поиск записи по ключу сразу в 3 типах таблиц.
	{
		FindResult resulttUnordered = unorderedTable.find(key);
		logger.add(unorderedTable.getTableName(), "find", resulttUnordered.operationsCount);
		...
		result.operationsCount = resulttUnordered.operationsCount + resultAVL.operationsCount + resultHash.operationsCount; // Количество затраченных операций в сумме.
	...
	size_t removeALL(const TKey& key) // Удаление записи по ключу сразу в 3 типах таблиц.
	{
	...
	{
		return (unorderedTable.size() == 0 && avlTable.size() == 0 && hashTable.size() == 0);
	}
