...
std::string
input_key() // Функция для ввода ключа для данной записи пользователем.
{
  std::string user_key = ""; // Буфер ввода пользователем.
  std::cout << "Enter the key for this record:\n";

  std::getline(std::cin,
               user_key); // std::getline() считает строку из стандартного
                          // потока ввода (std::cin) до первого символа новой
                          // строки ('\n') и сохраняет её в переменной user_key.
  return user_key;
}

void saveResultOfOperationWithPolynomials(
    Polynomial &polynom, MainTable<std::string, Polynomial>
                             &table) // Функция сохранения результатов операций
                                     // с полиномами в таблицу.
{
  std::cout << "0 - put the result of adding polynomials in a table (insert), "
               "1 - do nothing.\nEnter:\n";
  std::string user_input_save = ""; // Буфер ввода пользователем.

  bool flag_save = true; // Флаг для выполнения команды до тех пор, пока не
                         // будет введено верное значение выбора.
  int temp_save = -1; // Результат выбора пользователем.
  while (flag_save) {
    std::getline(std::cin,
                 user_input_save); // std::getline() считает строку из
                                   // стандартного потока ввода (std::cin) до
                                   // первого символа новой строки ('\n') и
                                   // сохраняет её в переменной user_input_save.
    try {
      temp_save = std::stoi(
          user_input_save); // std::stoi() преобразует строку user_input_save в
                            // целое число (int). Если строка не может быть
                            // преобразована в число, будет выброшено исключение
                            // std::invalid_argument. Если число слишком велико
                            // для типа int, будет выброшено исключение
                            // std::out_of_range.
                           ...
                           // (из лабораторной работы #1).
{
  ...
  bool flag_input =
      true; // Флаг для ввода пользователем всех нужных ему мономов.

  std::vector<std::pair<double, size_t>>
      source; // Вектор для хранения данных первого полинома, который будет
              // передан в конструктор класса Polynomial для создания первого
              // полинома.
  ...
  while (flag_input) {
    ...
    std::getline(
        std::cin,
        user_input); // std::getline() считает строку из стандартного потока
                     // ввода (std::cin) до первого символа новой строки ('\n')
                     // и сохраняет её в переменной user_input.
    try {
      temp_coeff = std::stod(
          user_input); // std::stod() преобразует строку user_input в число с
                       // плавающей запятой (double). Если строка не может быть
                       // преобразована в число, будет выброшено исключение
                       // std::invalid_argument. Если число слишком велико для
                       // типа double, будет выброшено исключение
                       // std::out_of_range.

      if (std::fabs(temp_coeff) <
          EPSILON) // Проверка на равенство коэффциеинта 0.
      {
    ...
    try {
      temp_deg = std::stoull(
          user_input); // std::stoull() преобразует строку user_input в
                       // беззнаковое целое число (unsigned long long). Если
                       // строка не может быть преобразована в число, будет
                       // выброшено исключение std::invalid_argument. Если число
                       // слишком велико для типа unsigned long long, будет
                       // выброшено исключение std::out_of_range.

      if (temp_deg > 999 || temp_deg < 0) // Проверка на выход значения степени
                                          // монома за установленные пределы.
      {
    ...
    source.push_back(
        {temp_coeff, temp_deg}); // Добавляем моном в вектор source для
                                 // последующей записи в полином.
    ...
    std::cout << "1 - Continue entering the polynomial, 0 - Finish entering "
                 "the polynomial.\nEnter:\n";
    bool flag_temp =
        true; // Флаг для выбора пользователем нужного варианта ответа.
    while (flag_temp) {
      ...
      try {
        temp = std::stoi(
            user_input); // std::stoi() преобразует строку user_input в целое
                         // число (int). Если строка не может быть преобразована
                         // в число, будет выброшено исключение
                         // std::invalid_argument. Если число слишком велико для
                         // типа int, будет выброшено исключение
                         // std::out_of_range.
  ...
  Polynomial temp_p1(
      source); // Создаём полином с данными, которые ввёл пользователь (source).
...
int input_value_int() // Ввод значения integer с консоли.
{
  ...
  while (true) {
    ...
    try {
      int result =
          std::stoi(user_input); // std::stoi() преобразует строку user_input в
                                 // целое число (int). если строка не может быть
                                 // преобразована в число, будет выброшено
                                 // исключение std::invalid_argument. если число
                                 // слишком велико для типа int, будет выброшено
...
std::string input_value_string() // Ввод значения std::string с консоли.
{
  ...
  std::getline(std::cin,
               result); // std::getline() считает строку из стандартного потока
                        // ввода (std::cin) до первого символа новой строки
                        // ('\n') и сохраняет её в переменной user_input.
  return result; // Возвращаем результат ввода пользователем.
}
...
void insertToTable(
    MainTable<TKey, TValue> &table_,
    TValue (*input_func)()) // Шаблонная функция для универсальной вставки
                            // записи в таблицу (таблицы). TValue(*input_func)()
                            // - объявление указателя на функцию, для вызова
                            // функций, возвращающих разные TValue: так сможем
...
std::ostream &operator<<(
    std::ostream &out,
    const Polynomial &polynom_) // Перегруженный оператор вывода полиномов.
{
...
void findInTable(
    MainTable<TKey, TValue> &table_,
    const bool &outputFactor_) // Шаблонная функция для универсального поиска
                               // записи в таблице (таблицах) по ключу.
{
  ...
  TValue *value =
      table_.findALL(key)
          .value; // Получаем результат поиска записи по ключу: указатель на
                  // значение записи или nullptr (если не найдена запись).
  if (value != nullptr) {
    std::cout << "The search is completed: " << *value << std::endl;
    if (outputFactor_) // Проверка на то, хочет ли пользователь извлечь
                       // (получить и удалить) значение записи.
    {
...
void removeFromTable(
    MainTable<TKey, TValue>
        &table_) // Шаблонная функция для универсального удаления записи по
                 // ключу из таблицы (таблиц).
{
  if (table_.is_emptyALL()) // Проверка на пустоту таблицы.
  {
    std::cout << "The table is empty." << std::endl;
...
int main() {
  ...
  Polynomial p1, p2, res_p; // p1, p2 - полиномы для ввода пользователем; res_p
  ...
  bool flag_type = true; // Флаг для выполнения команды до тех пор, пока не
                         // будет введено верное значение выбора.
  int temp_type = -1; // Результат выбора пользователем.
  while (flag_type) {
    std::getline(std::cin,
                 user_input_type); // std::getline() считает строку из
                                   // стандартного потока ввода (std::cin) до
                                   // первого символа новой строки ('\n') и
                                   // сохраняет её в переменной user_input_type.
    try {
      temp_type = std::stoi(
          user_input_type); // std::stoi() преобразует строку user_input_type в
                            // целое число (int). Если строка не может быть
                            // преобразована в число, будет выброшено исключение
                            // std::invalid_argument. Если число слишком велико
                            // для типа int, будет выброшено исключение
  ...
  while (flag) {
    ...
    std::getline(std::cin,
                 choice); // std::getline() считает строку из стандартного
                          // потока ввода (std::cin) до первого символа новой
    ...
    try {
      num_choice = std::stoi(
          choice); // std::stoi() преобразует строку user_input в целое число
                   // (int). Если строка не может быть преобразована в число,
                   // будет выброшено исключение std::invalid_argument. Если
                   // число слишком велико для типа int, будет выброшено
                   // исключение std::out_of_range.
    } catch (std::exception) {
      std::cout << "INVALID INPUT. Please enter a number from 0 to 10\n";
      continue; // Переход к началу цикла, чтобы пользователь мог ввести
                // правильное значение команды.
    }
    ...
    case 3: {
      if (!p1.is_empty() &&
          !p2.is_empty()) // Если полиномы не пусты, то выполняем команду.
      {
        res_p =
            p1 +
            p2; // Используем перегруженные операторы сложения и присваивания.
        res_p.print_polynom();

        if (typeValue == POLYNOMIAL) {
          saveResultOfOperationWithPolynomials(
              res_p,
              tablePolynomial); // Сохранение результата операции с полиномами.
        }
      } else {
        if (p1.is_empty()) {
    ...
    case 4: {
          ...
          !p2.is_empty()) // Если полиномы не пусты, то выполняем команду.
      {
        ...
        bool flag_temp = true; // Флаг для выполнения команды до тех пор, пока
                               // не будет введено верное значение выбора.
        int temp = -1; // Результат выбора пользователем.
        while (flag_temp) {
          std::getline(std::cin,
                       user_input); // std::getline() считает строку из
                                    // стандартного потока ввода (std::cin) до
                                    // первого символа новой строки ('\n') и
                                    // сохраняет её в переменной user_input.
          try {
            temp = std::stoi(
                user_input); // std::stoi() преобразует строку user_input в
                             // целое число (int). Если строка не может быть
                             // преобразована в число, будет выброшено
                             // исключение std::invalid_argument. Если число
                             // слишком велико для типа int, будет выброшено
            ...
            if (temp == 0 || temp == 1) {
              if (temp == 0) {
                res_p = p1 - p2; // Используем перегруженные операторы вычитания
                                 // и присваивания.
                res_p.print_polynom();
              } else {
                res_p = p2 - p1; // Используем перегруженные операторы вычитания
                                 // и присваивания.
                res_p.print_polynom();
              }
              ...
              if (typeValue == POLYNOMIAL) {
                saveResultOfOperationWithPolynomials(
                    res_p, tablePolynomial); // Сохранение результата операции с
                                             // полиномами.
              }
              flag_temp = false;
            } else {
    ...
    case 5: {
      ...
      bool flag_temp = true; // Флаг для выполнения команды до тех пор, пока не
                             // будет введено верное значение выбора.
      int temp = -1; // Результат выбора пользователем.
      ...
      while (flag_temp) {
        ...
        try {
          temp = std::stoi(
              user_input); // std::stoi() преобразует строку user_input в целое
                           // число (int). Если строка не может быть
                           // преобразована в число, будет выброшено исключение
                           // std::invalid_argument. Если число слишком велико
                           // для типа int, будет выброшено исключение
                           // std::out_of_range.
          ...
          if (temp == 0 && !p1.is_empty() || temp == 1 && !p2.is_empty()) {
            ...
            bool flag_const =
                true; // Флаг для выполнения команды до тех пор, пока не будет
                      // введено верное значение выбора.
            while (flag_const) {
              std::getline(
                  std::cin,
                  user_input); // std::getline() считает строку из стандартного
                               // потока ввода (std::cin) до первого символа
                               // новой строки ('\n') и сохраняет её в
                               // переменной user_input.
              try {
                temp_const = std::stod(
                    user_input); // std::stod() преобразует строку user_input в
                                 // число с плавающей запятой (double). Если
                                 // строка не может быть преобразована в число,
                                 // будет выброшено исключение
                                 // std::invalid_argument. Если число слишком
                                 // велико для типа double, будет выброшено
                                 // исключение std::out_of_range.
                flag_const = false;
              } catch (std::exception) {
    ...
    case 6: {
      ...
      {
        try {
          res_p = p1 * p2; // Используем перегруженные операторы умножения и
                           // присваивания.
          res_p.print_polynom();
    ...
    case 7: {
      ...
      case INTEGER: {
        insertToTable(
            tableInt,
            input_value_int); // Вызываем универсальную функцию вставки.
        break;
      }
      case DOUBLE: {
        insertToTable(
            tableDouble,
            input_value_double); // Вызываем универсальную функцию вставки.
        break;
      }
      case STRING: {
        insertToTable(
            tableString,
            input_value_string); // Вызываем универсальную функцию вставки.
        break;
      }
      case POLYNOMIAL: {
        insertToTable(tablePolynomial,
                      input_polynom); // Вызываем универсальную функцию вставки.
        break;
      }
      case BOOL: {
        insertToTable(
            tableBool,
            input_value_bool); // Вызываем универсальную функцию вставки.
        break;
      }
      case CHAR: {
        insertToTable(
            tableChar,
            input_value_char); // Вызываем универсальную функцию вставки.
        break;
      }
      }
    ...
    case 8: {
      ...
      bool outputFactor =
          false; // Фактор вывода данных записи: false - только выводить; true -
                 // вывести и удалить запись (извлечь).
      ...
      switch (typeValue) {
      case INTEGER: {
        findInTable(tableInt, outputFactor); // Вызываем универсальную функцию
                                             // поиска записи по ключу.
        break;
      }
      case DOUBLE: {
        findInTable(tableDouble,
                    outputFactor); // Вызываем универсальную функцию поиска
                                   // записи по ключу.
        break;
      }
      case STRING: {
        findInTable(tableString,
                    outputFactor); // Вызываем универсальную функцию поиска
                                   // записи по ключу.
        break;
      }
      case POLYNOMIAL: {
        findInTable(tablePolynomial,
                    outputFactor); // Вызываем универсальную функцию поиска
                                   // записи по ключу.
        break;
      }
      case BOOL: {
        findInTable(tableBool, outputFactor); // Вызываем универсальную функцию
                                              // поиска записи по ключу.
        break;
      }
      case CHAR: {
        findInTable(tableChar, outputFactor); // Вызываем универсальную функцию
                                              // поиска записи по ключу.
        break;
      }
      }
    ...
    case 0: {
      ...
      std::getline(
          std::cin,
          file_name); // std::getline() считает строку из стандартного потока
                      // ввода (std::cin) до первого символа новой строки ('\n')
                      // и сохраняет её в переменной user_key.
