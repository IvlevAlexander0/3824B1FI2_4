...
void Polynomial::record() {
    std::cout
        << "enter the monomials step by step. General view of the monome K * "
        "x^l1 * y^l2 * z^l3\nl1,l2,l3 - (0 - 9), k - the real coefficient"
        << "To stop typing, write K = 0" << std::endl;
    int i = 1;
    int sum_deg;
    double rat;
    int l1, l2, l3, degree;
        ...
        if (!flage) {
            for (auto it = view.begin(); it != view.end(); ++it) {
                ...
                if (curr->_next == nullptr) {
                    ...
                    if (sum_numb(monome.second) > sum_numb(nextcur->_val.second) &&
                        sum_numb(monome.second) < sum_numb(curr->_val.second)) {
    ...
    for (it1; it1 != view.end(); it1++) {
        for (auto it2 = A.view.begin(); it2 != view.end(); it2++) {
            ...
            if (chek_deg(deg, (*it1).second) || chek_deg(deg, (*it2).second)) {
                ...
                bool found = true;
                for (auto resIt = new_Polynomial.view.begin();
                    resIt != new_Polynomial.view.end(); ++resIt) {
                    ...
                    if ((*resIt).second == new_pair.second) {
                        ...
                        if (sum_numb(new_pair.second) > sum_numb(nextcur->_val.second) &&
                            sum_numb(new_pair.second) < sum_numb(curr->_val.second)) {
...
const Polynomial operator*(double val, const Polynomial& A) { return A * val; }
