...
template <class T>
class TableVector : public Table<T> {
...
template<class T>
TableVector<T>::TableVector(size_t size): _countOper(0) { _data.reserve(size); }

template<class T>
TableVector<T>::~TableVector() { _data.clear(); }
...
template<class T>
T* TableVector<T>::find(const std::string& key) {
	_countOper ++;
	for (int i = 0; i < _data.size(); i++) {
		_countOper += 4;
		if (_data[i].first == key)
...
template<class T>
void TableVector<T>::remove(const std::string& key) {
	_countOper++;
	for (int i = 0; i < _data.size(); i++) {
		_countOper += 4;
		if (_data[i].first == key) {
...
template<class T>
int TableVector<T>::getCountOper() const{
	return _countOper;
}
