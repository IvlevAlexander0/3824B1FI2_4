...
struct NodeAVL
{
	NodeAVL(std::pair<std::string, T> data, NodeAVL<T>* left = nullptr, NodeAVL<T>* right = nullptr);
	NodeAVL<T>* _left;
	NodeAVL<T>* _right;
	int _height;
...
template<class T>
NodeAVL<T>::NodeAVL(std::pair<std::string, T> data, NodeAVL<T>* left, NodeAVL<T>* right) : _right(right), _left(left), _data(data), _height(1) {}
...
template<class T>
NodeAVL<T>* TableAVL<T>::getRight(NodeAVL<T>* node) const {
	if (node != nullptr) {
		return node->_right;
	}
	return nullptr;
...
template<class T>
NodeAVL<T>* TableAVL<T>::getLeft(NodeAVL<T>* node) const {
	if (node != nullptr) {
		return node->_left;
	}
	return nullptr;
...
template<class T>
std::string TableAVL<T>::getKey(NodeAVL<T>* node) const {
	if (node != nullptr) {
...
template<class T>
int TableAVL<T>::height(NodeAVL<T>* node) {
	_countOper++;
	if (node != nullptr) 
		return node->_height;
	else 
...
template<class T>
int TableAVL<T>::balanceRaz(NodeAVL<T>* node) {
	_countOper++;
	return height(node->_left) - height(node->_right);
}

template<class T>
void TableAVL<T>::updateHeight(NodeAVL<T>* node) {
	if (node) {
...
template<class T>
NodeAVL<T>* TableAVL<T>::small_right_turn(NodeAVL<T>* node) {
	_countOper += 3;  
...
template<class T>
NodeAVL<T>* TableAVL<T>::small_left_turn(NodeAVL<T>* node) {
	_countOper += 3;
...
template<class T>
NodeAVL<T>* TableAVL<T>::big_right_turn(NodeAVL<T>* node) {
	_countOper++;
...
template<class T>
NodeAVL<T>* TableAVL<T>::big_left_turn(NodeAVL<T>* node) {
	_countOper++;
...
template<class T>
NodeAVL<T>* TableAVL<T>::balance(NodeAVL<T>* node) {
	_countOper++;
	if (!node) 
		return nullptr;
...
template<class T>
TableAVL<T>::TableAVL() : _countOper(0), _head(nullptr) {};
template<class T>
TableAVL<T>::~TableAVL() {
	clear(_head);
}
...
template<class T>
NodeAVL<T>* TableAVL<T>::insertRec(NodeAVL<T>* node, const std::string& key, const T& val) {
	_countOper++;
	if (node == nullptr) {
...
template<class T>
NodeAVL<T>* TableAVL<T>::findMin(NodeAVL<T>* node) {
	_countOper++;
	if (!node) 
		return nullptr;
...
template<class T>
void TableAVL<T>::remove(const std::string& key) {
	_head = removeRec(_head, key);
}

template<class T>
void TableAVL<T>::clear(NodeAVL<T>* node) {
	if (node != nullptr) {
		clear(node->_left);
		clear(node->_right);
		delete node;
...
template<class T>
T* TableAVL<T>::find(const std::string& key) {
	return findRec(_head, key);
}

template<class T>
int TableAVL<T>::getCountOper() const { return _countOper; }
